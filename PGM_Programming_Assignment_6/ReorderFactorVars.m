function F2 = ReorderFactorVars(F, newVarOrder)
  ...
  [temp, newP] = ismember(newVarOrder,F.var); % newP is the positional permutation of F with inverse permute
  F2.var = newVarOrder;
  F2.card = F.card(newP);
  % to match, we take F2's assignment table and reorder columns by permute
  assign = IndexToAssignment([1:prod(F2.card)],F2.card);
  newAssign = assign(:,permute);
